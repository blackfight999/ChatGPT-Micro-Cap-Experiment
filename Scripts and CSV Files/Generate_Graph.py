...
# === Download and prepare Bitcoin ===
start_date = baseline_date
end_date = chatgpt_totals['Date'].max()

btc = yf.download("BTC-USD", start=start_date, end=end_date + pd.Timedelta(days=1), progress=False)
btc = btc.reset_index()
...
btc_27_price = btc["Close"].iloc[0].item()
btc_scaling_factor = 100 / btc_27_price
btc["BTC Value ($100 Invested)"] = btc["Close"] * btc_scaling_factor
...
plt.plot(btc['Date'], btc["BTC Value ($100 Invested)"], label="Bitcoin ($100 Invested)", marker="o", color="orange", linestyle='--', linewidth=2)

final_date = chatgpt_totals['Date'].iloc[-1]
final_chatgpt = float(chatgpt_totals["Total Equity"].iloc[-1])
final_btc = btc["BTC Value ($100 Invested)"].iloc[-1]

plt.text(final_date, final_chatgpt + 0.3, f"+{final_chatgpt - 100:.1f}%", color="blue", fontsize=9)
plt.text(final_date, final_btc + 0.9, f"+{final_btc - 100:.1f}%", color="orange", fontsize=9)
...
plt.title("ChatGPT's Crypto Portfolio vs. Bitcoin")
plt.xlabel("Date")
